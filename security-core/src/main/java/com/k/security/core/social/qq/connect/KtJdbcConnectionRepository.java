 ...
 * Time: 12:01
 */
public class KtJdbcConnectionRepository implements ConnectionRepository {
    private final String userId;
    private final JdbcTemplate jdbcTemplate;
    private final ConnectionFactoryLocator connectionFactoryLocator;
    private final TextEncryptor textEncryptor;
    private final String tablePrefix;
    private final KtJdbcConnectionRepository.ServiceProviderConnectionMapper connectionMapper =
            new KtJdbcConnectionRepository.ServiceProviderConnectionMapper();

    public KtJdbcConnectionRepository(String userId, JdbcTemplate jdbcTemplate,
                                      ConnectionFactoryLocator connectionFactoryLocator,
                                      TextEncryptor textEncryptor, String tablePrefix) {
        this.userId = userId;
    ...
    private Connection<?> findPrimaryConnection(String providerId) {
        List<Connection<?>> connections = this.jdbcTemplate.query(this.selectFromUserConnection()
                + " where userId = ? and providerId = ? order by `rank`", this.connectionMapper,
                new Object[]{this.userId, providerId});
    ...
    private final class ServiceProviderConnectionMapper implements RowMapper<Connection<?>> {
        ...
        public Connection<?> mapRow(ResultSet rs, int rowNum) throws SQLException {
            ...
            ConnectionFactory<?> connectionFactory = KtJdbcConnectionRepository.this.connectionFactoryLocator
        ...
        private ConnectionData mapConnectionData(ResultSet rs) throws SQLException {
            return new ConnectionData(rs.getString("providerId"), rs.getString("providerUserId"),
                    ...
                    rs.getString("imageUrl"), this.decrypt(rs.getString("accessToken")),
                    this.decrypt(rs.getString("secret")),
                    this.decrypt(rs.getString("refreshToken")),
                    this.expireTime(rs.getLong("expireTime")));
        }

        private String decrypt(String encryptedText) {
            return encryptedText != null ?
                    KtJdbcConnectionRepository.this.textEncryptor.decrypt(encryptedText) : encryptedText;
        }
